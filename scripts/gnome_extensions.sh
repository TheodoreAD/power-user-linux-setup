...
function get_uri_last_token_without_extension() {
    repo_url=$1
    # echo -n "$(echo "$1" | rev | cut -d ' ' -f 1 | rev)"
    echo -n "$(echo ${repo_url##*/} | cut -d . -f 1)"
}
...
# TODO: description
#curl -sS -SMTHING https://extensions.gnome.org/extension-data/world_clock_liteailin.nemui.v9.shell-extension.zip
...
# to use the extension right away
# warning: this causes some scaled windows to display as if unscaled
# alt+f2 -> r -> enter
# supposedly good from the command line, crashed the system
#gnome-shell --replace


# other extensions available in apt
