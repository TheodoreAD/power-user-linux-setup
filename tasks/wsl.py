...
def _resolv_conf_symlinked_to_stub() -> bool:
    ...
    by WSL" comment, since nothing has rewritten it) is left in place. `inv system.configure-dns` only
...
def check(c: Context):  # noqa: C901
    ...
    # deb-github, deb-url methods) and Ubuntu specifically for apt.install_repos (uses `lsb_release -cs` as
    # the codename in repo source lines) and apt.install_base's package set.
    ...
    # systemd — gates system.set_locale, system.configure_dns, system.cap_journal_size, docker.configure
    if _systemd_running():
    ...
    if _resolv_conf_wsl_active():
        print(
            "[wsl] resolv.conf: auto-generated by WSL  ← inv system.configure-dns writes a systemd-resolved "
        ...
        print("[wsl] resolv.conf: not WSL-managed ✓ (inv system.configure-dns is safe to use)")
    else:
        print(
            "[wsl] resolv.conf: generateResolvConf=false is set, but /etc/resolv.conf isn't a "
            "symlink to systemd-resolved's stub yet  ← inv system.configure-dns edits systemd-resolved's own "
            ...
            f"       Fix: `{relink_cmd}` then `inv system.configure-dns`."
        )
...
def fix(c: Context, dns: bool = False):
    ...
    dns: pass --dns to set generateResolvConf=false instead, overriding WSL's own DNS with public
      resolvers (1.1.1.1 etc.) via systemd-resolved/inv system.configure-dns. Defaults to leaving WSL's own
    ...
    if dns_changed and dns:
        print(
            ...
            f"`sudo ln -sf {_STUB_RESOLV_CONF} /etc/resolv.conf` then `inv system.configure-dns` "
        ...
        print(
            ...
            "Windows host's own DNS settings — nothing else to run, don't call `inv system.configure-dns`."
        )
...
def install(c: Context, wslg: str = "auto", docker: bool = False, dns: str = "auto"):  # noqa: C901
    ...
    # Authenticate sudo once, before anything installs, and keep the credential cache warm for the
    # rest of the run so no later step stops to ask (see util.ensure_sudo). This used to be
    # `c.run("sudo -v", pty=True)`, which hangs outright on Python 3.14 — invoke can't forward
    # stdin there at all (pyinvoke/invoke#1070), so the prompt appeared and nothing typed ever
    # reached it.
    util.ensure_sudo("installing packages and editing /etc/wsl.conf")
    ...
    phases.run_phase(
        c,
        "shell",
        [zsh.configure_omz, zsh.configure, zsh.configure_p10k, zsh.set_default_shell],
