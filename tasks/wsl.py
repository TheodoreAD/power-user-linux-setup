...
def check(c):
    ...
    if _resolv_conf_is_wsl_generated():
        ...
        if already_disabled:
            ...
            print(
                ...
                "`wsl.exe --shutdown`, reopen, delete the stale /etc/resolv.conf, and re-run "
                "`inv system.dns`."
            )
    else:
    ...
    if _wslg_available():
        print(
            "[wsl] WSLg: available ✓ — wl-clipboard and gui/desktop-tagged packages work normally, "
            "but install as little as possible: PULSE_EXCLUDE_TAGS=gnome,ide,windows-native,"
            "workstation,corporate\n"
            "       This keeps just wl-clipboard (interop) and Chrome (real-Linux-build testing). "
            "`ide` (vscode, jetbrains-toolbox) and `windows-native` (terminator, wezterm, freelens, "
            "font-manager, claude-desktop, edge) all work under WSLg too, but duplicate a Windows-"
            "native app with no Linux-specific reason to run the Linux build — see docs/wsl.md, "
            "\"IDEs: edit from Windows instead\" and \"Windows-native duplicates\"."
        )
    else:
        print(
...
def fix(c):
    ...
    if dns_changed:
        print(
            "[wsl.fix] After restarting: /etc/resolv.conf may still show the old WSL-generated "
            "content until you delete it and re-run `inv system.dns` — see docs/wsl.md."
        )
