...
**1. The package catalog.** `docs/packages.md`, 97 rows, generated by `inv catalog.render-packages`
from `setup.toml` and guarded by a drift test. Two entries are excluded and the page says so:
anything `enabled = false`, and the one entry with no `method` at all (`[packages.repo-tasks]`
carries a permission grant and installs nothing).
...
**6. The task index and the updating page.** `docs/tasks.md` is generated from the invoke namespace
by `inv catalog.render-tasks` (132 tasks, drift-tested), which makes it the second generator and the
reason `catalog.render` became `catalog.render-packages`. `docs/updating.md` collects what was
spread across five pages — what apt upgrades and what does not, cache reclamation, and removal
including `cleanup_paths` and the reboot Citrix needs. `docs/index.md`'s Maintenance section shrank
to three commands and a link, since it had become a second copy of that material.

[DECISION: task summaries are **clipped**, where package descriptions were fixed at the source
instead. Thirteen of 132 first sentences run past 200 characters and two belong to `repo-tasks`,
which this repo may not edit — so "fix every source" was not available. The clip prefers the
sentence's own first clause (these docstrings almost all open with a summary, an em dash, then the
detail) and hard-cuts only when that clause is a fragment. Ten of the thirteen take the clause
path.]

**4. Openers and `## See also`.** Sixteen pages opened on a heading, a bare URL or an admonition;
all now open with one line naming the subject. Thirty-eight had no footer; all now carry two to four
links chosen for what a reader of that page wants next.

[PITFALL: the sweep inserted openers by script and dropped the blank line after them, so dprint
folded the next block into the paragraph — inlining the admonition on `kubernetes.md`/`gcloud.md`
and the autolinks on `golang.md`/`rust.md`/`js.md`. **`inv quality.precommit` and
`zensical build --strict` both stayed green**, which `CONTRIBUTING.md` already warned about: an
admonition that loses its blank line is a lint-clean, build-clean, silently wrong page. Caught by
reading the files afterwards, and confirmed fixed in the built HTML rather than in the source. A
