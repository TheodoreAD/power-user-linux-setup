...
[NEEDS CLARIFICATION: does the rule mean "write 3.11-compatible syntax" or "declare
`requires-python = ">=3.11"`"? For a packaged library they coincide. For `agent-skills`' stdlib
scripts, run as a bare `python3` on whatever the machine has, they do not: that interpreter is 3.12
on Ubuntu 24.04 and 3.10 on 22.04, so a 3.11 floor there is a claim about which machines a skill
runs on rather than about what a resolver will accept. The wording has to survive that case.]
...
2. Put the two measurements above in `contributing/global-agents-md.md` under a matching heading.
