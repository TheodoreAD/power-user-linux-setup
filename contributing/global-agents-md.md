...
(the measured comparison behind those values is in `docs/locale.md`), so a rule opening "this
machine's `LC_TIME` defaults to `ro_RO`" asserted something false into every session. The
instruction was always the portable half — forcing the C locale for output a script parses is
correct wherever it runs, and merely unnecessary on a single-locale machine — so what changed is
which half leads. The two concrete tells stay in the rule (`Mi` for `Wed`, `1,23` for `1.23`)
because they are what makes the hazard recognisable; the incident that produced them stays here.
