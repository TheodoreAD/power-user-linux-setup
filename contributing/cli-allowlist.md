...
Companion to [`docs/cli-allowlist.md`](../docs/cli-allowlist.md) (the published "what it is / how
to run it" page). This is the durable explanation of _why_ `cli-allowlist/` and `tasks/allowlist.py`
are built the way they are, including every gotcha the first implementation pass hit and how
testing (not code review) caught each one — read this before re-deriving the architecture from
scratch or "fixing" something that was already a deliberate tradeoff.
...
- **`help_style = "prefix"`** — some tools put the flag _before_ the subcommand
...
1. **Nested help text often doesn't look like the top level's.** `git`'s top-level `-h` has a
   ...
   synopsis parser (`_discover_from_synopsis`) only when that finds nothing _and_ a tool+path
   ...
   (possibly-nested) `[...]` bracket groups, and takes the next token _only if it's a bare
   lowercase identifier_ — a positional-arg placeholder right after the known path
...
4. **Some extraction weirdness doesn't show up as duplicate content at all**, and no deterministic
   ...
   layer at all: each new shape of extraction weirdness found so far has been a _different_ one,
...
flags that plausibly _escalate_ risk (`force`, `recursive`, `all`, `yes`, `hard`, `global`,
`cascade`, ..., extending the same `_DANGEROUS_VERBS` set already used for the subcommand-name
backstop) and one for flags that plausibly _de-escalate_ it (`dry-run`, `check`, `plan-only`,
...
PreToolUse hook, which unlike a prefix glob _can_ parse flags order-independently — deliberately
...
- **A deterministic safety backstop runs after every LLM call, at no extra cost**: any subcommand
  ...
  in the subcommand _name itself_ (`delete`, `destroy`, `rm`, `force`, `run`, `exec`, ...) — a
  ...
  not a verb, and `--all` on a listing command broadens what's _shown_, not what's destroyed. Left
...
`source` lives on each _node_, not on the tool as a whole — originally because of recursion (five
...
whole tool): `classify` diffs _per node_, so a new `docker network` child doesn't force
...
silently exclude any tool that hasn't been through this. This is a per-_tool_ gate, not per-node:
...
structure actually lives) is skipped entirely. Its read_only verdict describes the _bare_
...
a declarative `ask` rule, for a benefit (catching a dangerous command _before_ Claude even proposes
