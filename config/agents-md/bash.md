...
with `rg` (the note names `grep`/`find`; `~/.agents/AGENTS.md`'s `rg`-over-`grep` and
`fd`-over-`find` preferences still govern how you call it), and keep using Read/Edit/Write for
