...
fragment, never the deployed file, then `inv deploy.all --name <package>`. Which fragment owns what
is in that directory's `README.md`; each rule's evidence and the admission criteria a new rule must
pass are in the same repo's `contributing/global-agents-md.md`.
