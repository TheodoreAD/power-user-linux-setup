...
5. Run the repo's own lint/format/test commands before committing the reference fixes — editing
   many docstrings/comments in one pass is exactly the kind of change that quietly trips a
   line-length rule or similar, and it's cheap to catch immediately rather than in a later session.
...
1. Read the whole file and sort its sections into threads: fully implemented/verified content
   ...
   inaccurate now (a described design that was later replaced — drop it, it has no destination).
   A single legacy file routinely turns into zero, one, or several `plans/*.md` files, not one.
