...
filtering — the sudo/file-write side of `ensure_block`/`sudo_write` isn't covered),
...
just `cd` into the repo and run tests the plain way, same as any other Python project — no `uv run`
prefix, no manual activation:
